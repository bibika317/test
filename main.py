...
# Совместимость: если где-то остались импорты data.storage, подменяем их на storage
try:
    import storage as _storage  # локальный модуль в корне проекта
    data_pkg = types.ModuleType("data")
    data_pkg.storage = _storage
    sys.modules.setdefault("data", data_pkg)
    sys.modules.setdefault("data.storage", _storage)
except Exception:
    pass
